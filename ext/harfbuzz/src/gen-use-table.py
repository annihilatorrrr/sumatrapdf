...
headers = [[f.readline() for _ in range(2)] for j, f in enumerate(files)
           if j != 2]
for j in range(5, 7):
	for line in files[j]:
		line = line.rstrip()
...
def map_to_use(data):
	...
	for U,(UISC,UIPC,UGC,AJT,UBlock) in data.items():
		...
		assert len(values) == 1, f"{hex(U)} {UISC} {UGC} {AJT} {values}"
		USE = values[0]
		...
		assert (UIPC in [Not_Applicable, Visual_Order_Left] or U == 0x0F7F
		        or USE in use_positions), f"{hex(U)} {UIPC} {USE} {UISC} {UGC} {AJT}"

		if pos_mapping := use_positions.get(USE, None):
			values = [k for k,v in pos_mapping.items() if v and UIPC in v]
			assert len(values) == 1, f"{hex(U)} {UIPC} {USE} {UISC} {UGC} {AJT} {values}"
			USE = USE + values[0]
...
print (" *")
print(
    f" *   {sys.argv[0]} IndicSyllabicCategory.txt IndicPositionalCategory.txt UnicodeData.txt ArabicShaping.txt Blocks.txt IndicSyllabicCategory-Additional.txt IndicPositionalCategory-Additional.txt"
)
print (" *")
